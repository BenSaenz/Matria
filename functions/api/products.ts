...
export const onRequestGet: PagesFunction<Env> = async (context) => {
  ...
  return json({ section: 'products', data: payload.products, revision: payload.revision }, 200, context.request);
};
...
export const onRequestPut: PagesFunction<Env> = async (context) => {
  ...
  const revision = await replaceSection(context.env, 'products', parsed.data);
  return json({ ok: true, section: 'products', revision }, 200, context.request);
};
